...
- Claude Desktop / Cowork: connect CompanyOS itself through Add folder. Inspect the exact connected root, then request Cowork's supported deletion permission for that root only; Git needs to remove temporary files. If Documents is already the root and prevents connecting the child separately, start a task with only CompanyOS connected. Do not request broad Documents deletion or work around a denied permission. Approval may be needed in later sessions. Machine-global Claude Code skills are not sufficient: enable the equivalent skills/plugin through Cowork's supported interface and verify discovery. An explicit folder pointer supports manual loading but is not verified core skill installation. Keep one versioned source and record the delivery route as an app adapter. Secure authentication was blocked in the tested shell; do not call full Cowork setup supported until the secure-auth and remote-result checks pass.
