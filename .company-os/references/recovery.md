...
1. Verify local persistent shell access. Check/install Git, GitHub CLI, Node 22+, and npm/npx. Authenticate through gh auth login --web using HTTPS, then gh auth setup-git. Do not export credentials from the old machine.
...
5. Run configure without changing stored choices. It recreates personal/company remotes, explicit-push settings, and executable push guards. Git local configuration and installed hooks are not restored automatically by cloning.
6. Establish a repo-local author identity if needed. Run register to rebuild the local brain pointer. Reinstall selected skills from their recorded sources through the global skills flow. Do not copy old absolute machine paths or authentication tokens.
