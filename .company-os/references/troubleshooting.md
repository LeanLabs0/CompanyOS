...
1. Establish the actual host, execution identity, connected brain root, network access, and supported permissions. Follow [app setup](app-setup.md). If the session is Claude Cowork or otherwise lacks a host shell, stop setup and send the user to Claude Code, ChatGPT Work, Codex, Grok, or a local IDE; do not construct a file-bridge clone, Git bundle, or terminal-control workaround. File-bridge access does not prove a shell can access the folder. In an allowed app, confirm the brain is the local project or workspace, then test once.
